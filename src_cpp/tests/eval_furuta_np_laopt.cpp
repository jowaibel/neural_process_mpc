...
// The CasADi reference runs with cost.x_diff = 0, since the laopt OCP does
// not have the x_diff term yet (TODO(x_diff) in FurutaNPOcpEigen.hpp).
#include <algorithm>
#include <chrono>
#include <cmath>
#include <iostream>
...
double ocpObjective(Ocp& ocp, const StateTrajectory& x, const InputTrajectory& u, const Ocp::Param& p)
{
    ...
    double obj = 0.0;
    for (int k = 0; k < N; ++k) {
        obj += h * ocp.lagrange_term_impl(Ocp::State(x.col(k)), Ocp::Input(u.col(k)), p, t0, tf, double(k) / N);
    }
...
int main(int argc, char** argv)
{
    ...
    /* CasADi reference, with x_diff disabled to match the laopt OCP */
    ...
    params.cost.xDiff.assign(params.cost.xDiff.size(), 0.0);
    ...
    // -- 1. Cost check on an NP rollout --
    {
        ...
        casadi::MX xSym = casadi::MX::sym("x", N + 1, Ocp::NX);
        ...
        casadi::DM xVal = casadi::DM::zeros(N + 1, Ocp::NX);
        casadi::DM uVal = casadi::DM::zeros(N, Ocp::NU);
        for (int k = 0; k <= N; ++k) {
            for (int j = 0; j < Ocp::NX; ++j) { xVal(k, j) = x(j, k); }
        }
    ...
    const StateTrajectory xCasadi = fromCasadiTransposed<Ocp::NX, N + 1>(controller.lastX());
